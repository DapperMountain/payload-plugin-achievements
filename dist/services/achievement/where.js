...
export function userScopeWhere(userId, scopeId) {
    const clauses = [{ user: { equals: userId } }];
    if (scopeId)
 ...
 */
export function userScopeIncludingUnscopedWhere(userId, scopeId) {
    if (!scopeId)
        return userScopeWhere(userId, null);
    return {
        and: [
            { user: { equals: userId } },
            {

 ...
 */
export function validateRuleNode(rule: Record<string, unknown>): string | null {
  ...
  if (type === 'elapsed-since') {
    ...
    if (since === 'first-event') {
      ...
      // Host anchors (e.g. enrollment-started) are registered via extensions.metricAnchors.
...
export function assertRuleGroupValid(
  input: RuleGroup | Record<string, unknown>[] | null | undefined,
  label: string,
): void {

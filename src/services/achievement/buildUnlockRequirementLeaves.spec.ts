...
describe('buildUnlockRequirementLeaves', () => {
    ...
    const leaves = await buildUnlockRequirementLeaves({
      ...
      rules: {
        ...
        rules: [
          ...
          { type: 'achievement-complete', achievementSlug: 'path-complete', combinator: 'and', rules: [] },
          {
            type: 'metric-minimum',
            metric: { id: 'm1', name: 'Tenure days', slug: 'tenure-days' },
            minimum: 365,
            unit: 'days',
            since: 'enrollment-started',
            combinator: 'and',
            rules: [],
          },
        ],
  ...
  test('includes achievement-complete when asked', async () => {
    ...
    const payload = {
      find: async () => ({
        docs: [{ id: 'a1', slug: 'path-complete', name: 'Path complete', eligibilityRules: [], completionRules: [] }],
      }),
      findByID: async () => null,
    }
    const leaves = await buildUnlockRequirementLeaves({
      ...
      rules: [{ type: 'achievement-complete', achievementSlug: 'path-complete' }],
    })
  ...
  test('resolves catalog names by id when the relation is not populated', async () => {
    ...
    const payload = {
      findByID: async ({ id }: { id: string }) =>
        id === 'tier-1' ? { id: 'tier-1', name: 'Novice', slug: 'novice' } : null,
      find: async () => ({ docs: [] }),
    }
    const leaves = await buildUnlockRequirementLeaves({
      payload: payload as never,
    ...
    expect(leaves[0]).toMatchObject({
      ...
      met: false,
      progress: 0.4,
      relations: [{ field: 'tier', collectionKey: 'tiers', id: 'tier-1', slug: 'novice', name: 'Novice' }],
    })
  })
})

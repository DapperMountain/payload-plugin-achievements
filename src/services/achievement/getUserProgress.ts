...
function leanCatalog(value: unknown): LeanCatalog | null {
...
function leanGrant(doc: Record<string, unknown>) {
  return {
    id: doc.id,
...
function leanRequest(doc: Record<string, unknown>) {
  return {
    id: doc.id,
...
function mapPaginatedDocs<TDoc, TOut>(
  result: PaginatedDocs<TDoc>,
  mapDoc: (doc: TDoc) => TOut,
): PaginatedDocs<TOut> {
  return {
...
async function resolveTiersForMe(args: {
  ...
  for (const doc of grants.docs) {
    scopeIds.add(relationId((doc as { scope?: unknown }).scope))
  }
  if (scopeIds.size === 0) scopeIds.add(null)

  const tiers: LeanTier[] = []

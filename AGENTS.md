...
- `agents:sync` — copies rules + `payload-overrides` from `@dappermountain/agent-practices` and `@dappermountain/agent-payload`; ensures `.cursor` → `.agents` symlinks.
...
| `skills/payload-overrides/` | `@dappermountain/agent-payload` sync | No — re-sync |

...
3. Optional: `extensions.metricAnchors` for host-specific dates used by computed elapsed metrics / `elapsed-since` (e.g. product `grantedAt`). System seed does not depend on these keys.
...
achievementPlugin({
  ...
  extensions: {
    metricAnchors: {
      'enrollment-started': async ({ payload, userId, scopeId }) => {
        ...
        // Host-shaped: find the product membership row and return its grantedAt
...
Then seed a computed metric (`kind: 'computed'`, `compute: 'elapsed'`, `unit: 'days'`, `since: 'enrollment-started'`) and use `metric-minimum` in rules.

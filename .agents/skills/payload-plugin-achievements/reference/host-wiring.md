...
3. Optional: `extensions.metricAnchors` for host-specific dates used by computed elapsed metrics (e.g. product `grantedAt`). System seed does not depend on these keys. Prefer `{ label, resolve }` where `label` is a Payload Admin UI string (`OptionLabel`: locale map or `({ t }) => t('custom:…')`).
